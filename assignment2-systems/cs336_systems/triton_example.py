...
def weighted_sum_fwd(
    ...
    x_block_ptr = tl.make_block_ptr(
        x_ptr, # the initial location of the thing
        shape=(NUM_ROWS, D,), # how many elements in the view
        strides=(x_stride_row, x_stride_dim), # how wide each view is 
        offsets=(row_tile_idx * ROWS_TILE_SIZE, 0), # where x_block starts
...
def weighted_sum_backward(
    ...
    for i in range(tl.cdiv(D, D_TILE_SIZE)):
        ...
        # Reduce as many rows as possible for the grad weight result
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        input_shape = x.shape
        x = rearrange(x, "... d -> (...) d") # flatten the memory so its 2d
